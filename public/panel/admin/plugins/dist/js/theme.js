! function(e, s, i) {
    ...
    i(s).ready(function() {
        ...
        $('.input-images').each(function () {
            $(this).imageUploader({
                inputName: $(this).data('input-name'), // Use the data-input-name attribute
            });
        });
        
        // Ensure jQuery is loaded
        $(document).ready(function() {
            $('.input-images').on('change', function(event) {
                // Get the files from the input
                var files = event.target.files;

                // Retrieve allowed extensions from data attribute
                var allowedExtensionsStr = $(this).data('input-accept');

                alert(allowedExtensionsStr);

                // Convert allowed extensions string to array
                var allowedExtensions = [];
                if (allowedExtensionsStr) {
                    allowedExtensions = allowedExtensionsStr.split(',').map(function(ext) {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    if (allowedExtensions.length > 0 && $.inArray(fileExtension, allowedExtensions) === -1) {
                        isValid = false;
                ...
                // If invalid file type, clear the input
                if (!isValid) {
                    $(this).val('');
                }
            });
