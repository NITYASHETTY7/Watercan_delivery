...
<div class="table-responsive">
    @include('panel.admin.zone_pincodes.table')
</div>
@if ($zonePincodes->lastPage() > 1)
    <div class="card-footer">
        <div class="row align-items-center">
            <div class="col-lg-7 d-flex justify-content-start">
                <div class="pagination mobile-justify-center">
                    {{ @$zonePincodes->appends(request()->except('page'))->links() ?? '' }}
                </div>
            </div>
                ...
                <label class="d-flex align-items-center" for="jumpTo">
                    ...
                    <div class="d-flex align-items-center">
                        <x-input type="number" class="form-control" id="jumpTo" name="page"
                            value="{{ @$zonePincodes->currentPage() ?? '' }}" validation="empty" />
                        <div class="w-25 bg-gray py-2 pl-2 fw-700">/ {{ @$zonePincodes->lastPage() ?? '' }}</div>
                    </div>
                </label>
            </div>
        </div>
