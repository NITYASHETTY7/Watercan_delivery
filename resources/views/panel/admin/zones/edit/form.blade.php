<form class="forms-sample ajaxForm" method="POST"
    action="{{ route('panel.admin.zones.update', secureToken($zone->id)) }}">
    <div class="row">
        <!-- start message area-->
        ...
        <x-input name="id" placeholder="" type="hidden" tooltip="" regex="" validation="empty" value="{{ $zone->id ?? '' }}" />
        <x-input name="request_with" placeholder="" type="hidden" tooltip="" regex="" validation="empty" value="update" />

        <div class="col-md-8 mx-auto">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <h3> @lang('ui.zone') @lang('ui.details') </h3>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <x-input name="name" placeholder="{{ __('ui.enter') . ' ' . __('ui.name') }}" type="text" regex="name" validation="common_name" value="{{ $zone->name }}" />
...
@push('script')
    ...
    <script>
        $('.ajaxForm').on('submit', function(e) {
            ...
            var redirectUrl = "{{ url('admin/zones') }}" + "?branch_id=" + "{{ secureToken($zone->branch_id) }}";
