...
<form class="ajaxForm" method="POST" action="{{ route('panel.admin.zones.store') }}" autocomplete="off">
    @csrf
    <x-input name="request_with" placeholder="" type="hidden" tooltip="" regex="" validation="empty" value="create" />
    <x-input name="branch_id" placeholder="" type="hidden" tooltip="" regex="" validation="empty"
...
@push('script')
    ...
    <script>
        $('.ajaxForm').on('submit', function(e) {
            ...
            var redirectUrl = "{{ url('admin/zones') }}" + "?branch_id=" + "{{ request()->get('branch_id') }}";
