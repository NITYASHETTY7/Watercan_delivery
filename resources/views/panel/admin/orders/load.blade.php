<div class="table-controller mb-2" style="margin-top: -10px;">
    <div class="d-flex">
            <div class="mr-3">
...
<div class="table-responsive">
    @if(request()->get('type') == \App\Models\Order::TYPE_SUBSCRIPTION)
        @include('panel.admin.orders.subscription-table')
    @else
        @include('panel.admin.orders.table')
    @endif
</div>
                ...
                <label class="d-flex align-items-center" for="jumpTo">
                    ...
                    <div class="d-flex align-items-center">
                        <x-input type="number" class="form-control" id="jumpTo" name="page"
                            value="{{ @$orders->currentPage() ?? '' }}" validation="empty" />
                        <div class="w-25 bg-gray py-2 pl-2 fw-700">/ {{ @$orders->lastPage() ?? '' }}</div>
                    </div>
                </label>
            </div>
        </div>
