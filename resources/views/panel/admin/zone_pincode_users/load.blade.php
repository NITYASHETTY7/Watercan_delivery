...
<div class="table-controller mb-2 negative-margin">
...
<div class="table-responsive">
    @include('panel.admin.zone_pincode_users.table')
</div>
@if ($zonePincodeUsers->lastPage() > 1)
    <div class="card-footer">
        <div class="row align-items-center">
            <div class="col-lg-7 d-flex justify-content-start">
                <div class="pagination mobile-justify-center">
                    {{ @$zonePincodeUsers->appends(request()->except('page'))->links() ?? '' }}
                </div>
            </div>
                ...
                <label class="d-flex align-items-center" for="jumpTo">
                    ...
                    <div class="d-flex align-items-center">
                        <x-input type="number" class="form-control" id="jumpTo" name="page"
                            value="{{ @$zonePincodeUsers->currentPage() ?? '' }}" validation="empty" />
                        <div class="w-25 bg-gray py-2 pl-2 fw-700">/ {{ @$zonePincodeUsers->lastPage() ?? '' }}</div>
                    </div>
                </label>
            </div>
        </div>
